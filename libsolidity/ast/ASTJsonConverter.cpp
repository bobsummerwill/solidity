...
bool ASTJsonConverter::visit(FunctionDefinition const& _node)
{
	std::vector<pair<string, Json::Value>> attributes = {
		make_pair("name", _node.name()),
		make_pair("documentation", _node.documentation() ? Json::Value(*_node.documentation()->text()) : Json::nullValue),
...
bool ASTJsonConverter::visit(NatspecDocumentation const& _node)
{
	Json::Value text{*_node.text()};
	std::vector<pair<string, Json::Value>> attributes = {
		make_pair("text", text),
	};
	setJsonNode(_node, "Natspec", std::move(attributes));
	return false;
}
