...
void ContractCompiler::appendCallValueCheck()
{
	...
	// TODO: error message?
	m_context.appendConditionalRevert("Ether sent to non payable function");
}
...
void ContractCompiler::appendFunctionSelector(ContractDefinition const& _contract)
{
	...
	if (!fallback && !etherReceiver)
		m_context.appendRevert("Contract cannot receive Ether");
	else
	{
		if (etherReceiver)
		{
	...
	for (auto const& it: interfaceFunctions)
	{
		...
		if (_contract.isLibrary() && functionType->stateMutability() > StateMutability::View)
		{
			...
			m_context.appendConditionalRevert("Non view function called without DELEGATECALL");
		}
